...
@agent(names=("medior",), agent_type=AgentType.COORDINATOR)
class MediorAgent:
    @classmethod
    def instructions(cls) -> str:
        return cleandoc(
            """
            You are the Coordinator (Medior) in this multi-agent discussion room.

            Your role is to coordinate the research by prompting the Research Agents to debate, and then synthesizing their findings into a final response.
            ...
            3. You MUST NOT include your own handle (e.g. '@username/medior') in the mentions array of `band_send_message`. The platform rejects messages that mention the sender itself.
